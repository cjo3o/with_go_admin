...
function EventList() {
  ...
  const toggleStatus = async (id, currentStatus) => {
    const newStatus = currentStatus === '이벤트 진행중' ? '이벤트 종료' : '이벤트 진행중';
    const { error } = await supabase
        .from('withgo_event')
        ...
        <div className="card">
          ...
          <div className="table-wrapper">
            <table className="event-table common-table">
              ...
              <tbody>
              {events.map((event) => (
                  <tr key={event.id} className={event.status === '이벤트 진행중' ? 'event-active-row' : ''}>
                  <td>
                      <Checkbox
                    ...
                    <td>
                      <button
                          className={`btn-best ${event.status === '이벤트 진행중' ? 'orange' : 'blue'}`}
                          onClick={() => toggleStatus(event.id, event.status)}
                      >
                        {event.status === '이벤트 진행중' ? '이벤트 진행중' : '이벤트 종료'}
                      </button>
                    </td>
